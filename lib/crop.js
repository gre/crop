...
(function (crop, doc) {
    ...
    function updateMoveSelection(x,y) {
        ...
        if (s.x < 0) { s.l += s.x; s.x=0;}
        if (s.y < 0) { s.h += s.y; s.y=0;}

        if (s.x+s.l > contextimage.canvas.width) { s.l = contextimage.canvas.width - s.x;}
        if (s.y+s.h > contextimage.canvas.height) { s.h = contextimage.canvas.height - s.y;}
    ...
    function drawSize(x, y, w, h) {
        if (selection.mode != "move") {
            ...
            cs.strokeText(Math.abs(w) + "x" + Math.abs(h), x+7, y+7);
        }
    }
    ...
    crop.select = function (w, h) {
        ...
        if(imageloaded && selecting) {
            if (delta+w > c.width || delta+h > c.height) {
                resetSelection();
