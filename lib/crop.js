...
(function (crop, doc) {
    ...
    function initNewSelection(x, y, keepRatio) {
        selection = {
            keepRatio: x!=0 && y!=0 && keepRatio,
            x:x,
            y:y, 
            l:0, 
            h:0,
    ...
    function onMouseMove(evt) {
        ...
        var x = confinePointX(evt.pageX - canvasPos.left),
            y = confinePointY(evt.pageY - canvasPos.top);
    ...
    function internalCrop(w, h) {
        var s = selection,
        cNewW = s.l * imageWidth / cWidth,
        cNewH = s.h * imageHeight / cHeight,
        newW = w && w>0 ? w : cNewW,
        newH = h && h>0 ? h : cNewH;
        try {
            ratio = cNewW/cNewH;
    ...
    crop.select = function (w, h, keepRatio) {
        if(imageloaded && selecting) {
            if (w > imageWidth || h > imageHeight) {
                ...
                resetSelection();
                initNewSelection(xRatio*(imageWidth-w)/2, yRatio*(imageHeight-h)/2, keepRatio);
